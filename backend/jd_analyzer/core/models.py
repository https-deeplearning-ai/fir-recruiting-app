...
class JDRequirements(BaseModel):
    ...
    # NEW: Company discovery fields
    mentioned_companies: List[str] = Field(default_factory=list, description="Companies explicitly mentioned in JD")
    excluded_companies: List[str] = Field(default_factory=list, description="User's own companies to exclude from research (e.g., DLAI, AI Fund)")
    target_domain: Optional[str] = Field(default=None, description="Primary industry/domain (e.g., 'voice ai', 'fintech')")
    competitor_context: Optional[str] = Field(default=None, description="Industry context for competitor discovery")
