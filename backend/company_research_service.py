...
class CompanyResearchService:
    ...
    async def research_companies_for_jd(
        ...
        try:
            ...
            # Update with discovered companies
            company_names = [c.get("name") for c in discovered[:20]]  # First 20
            await self._update_session_status(jd_id, "running", {
                "phase": "discovery",
                "action": f"Discovered {len(discovered)} companies",
                "discovered_companies": company_names,
    ...
    async def discover_companies(
        ...
        # Method 1: Expand seed companies
        if seed_companies:
            for i, seed in enumerate(seed_companies[:5], 1):  # Limit to avoid too many searches
                if jd_id:
                    await self._update_session_status(jd_id, "running", {
                        "phase": "discovery",
                        "action": f"Searching competitors of {seed} ({i}/{min(len(seed_companies), 5)})..."
                    })
        ...
        if self.config.USE_TAVILY and self.tavily_api_key:
            ...
            for i, query in enumerate(search_queries[:3], 1):  # Limit searches
                if jd_id:
                    await self._update_session_status(jd_id, "running", {
                        "phase": "discovery",
                        "action": f"Web search: \"{query[:50]}...\" ({i}/3)"
                    })
                web_results = await self._search_web(query)
                companies.extend(await self._extract_companies_from_web(web_results))
        ...
        if jd_id:
            await self._update_session_status(jd_id, "running", {
                "phase": "discovery",
                "action": f"Enriching {len(unique_companies[:100])} companies..."
            })
    ...
    async def search_competitors_web(self, company_name: str) -> List[Dict[str, Any]]:
        ...
        for query in queries:
            ...
            companies = await self._extract_companies_from_web(results)
...
DO NOT include recruiting/hiring/talent fields. This is competitive intelligence, not recruiting."""
    ...
    def _generate_search_queries(self, jd_context: Dict[str, Any]) -> List[str]:
        ...
        # PRIORITY 2: Competitor expansion from seed companies
        if seed_companies and len(seed_companies) > 0:
            seed = seed_companies[0]  # Use first mentioned company
        ...
        print(f"  - seed_companies: {seed_companies}")
        print(f"[SEARCH QUERIES] Strategy: Domain-first with authoritative sources (G2, Capterra, Gartner, Crunchbase)")
        print(f"{'='*100}\n")
        # =========================================
    ...
    async def _extract_companies_from_web(self, web_results: Dict[str, Any]) -> List[Dict[str, Any]]:
...
If no clear company names are found, return an empty array: []"""
    ...
    async def _screen_companies(
        ...
        if jd_id:
            await self._update_session_status(jd_id, "running", {
                "phase": "screening",
                "action": f"Screening {len(companies)} companies for relevance..."
            })

        scores = await self.batch_screen_companies_gpt5(companies, jd_context)

        # Add scores to companies
        for i, company in enumerate(companies):
            if i < len(scores):
                company["screening_score"] = scores[i]

        # Sort by score and return top candidates
        companies.sort(key=lambda x: x.get("screening_score", 0), reverse=True)

        if jd_id:
            await self._update_session_status(jd_id, "running", {
                "phase": "screening",
                "action": f"Screening complete. Top candidate: {companies[0].get('name') if companies else 'None'}"
            })
