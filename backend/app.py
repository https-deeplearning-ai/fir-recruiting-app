...
def generate_assessment_prompt(profile_summary, user_prompt):
    """Generate the prompt for Claude to assess the LinkedIn profile"""
    
    base_prompt = f"""
You are an objective, evidence-first hiring assessor. You will read the provided LinkedIn profile JSON and the hiring criteria. Be strict and conservative: if evidence for a claim is not present in the profile (or in cited enrichment fields), mark it as uncertain and deduct points for missing required items.
...
- "recommend": true if you would recommend reaching out for this specific role (7+ score)
- "recommend": false if the candidate is not suitable/not strong enough for this role (1-6 score)

Please provide:
1. An overall profile strength score (1-10) based on fit for the specific role
2. Key strengths (in relation to the user's prompt) identified
3. Key weaknesses (in relation to the user's prompt) identified
4. Career trajectory analysis (in relation to the user's prompt)
5. Binary recommendation (true/false) for reaching out

Assessment Criteria: {user_prompt}
...
{{
    ...
    "detailed_analysis": "concise but comprehensive analysis based on user criteria"
}}
"""
...
def assess_profile():
    try:
        ...
        assessment_prompt = generate_assessment_prompt(profile_summary, user_prompt)
